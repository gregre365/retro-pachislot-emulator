...
/*************************************************************************
    ...
    flags and bonus state across an end-of-day shutdown or power failure.

    This build asserts that signal automatically: machine_start() registers
    power_off_requested() with running_machine::set_power_off_sequence(), a
    Retro Pachislot Emulator extension of the MAME core.  On the first exit
    request the core calls it instead of exiting, the signal appears on IN1 bit
    7 via shutdown_r(), the firmware saves its context and writes 0x20 to
    0x4008, and status_w() then calls power_off_complete() so the exit proceeds
    with a resumable context in the NVRAM file.  Further exit requests are
    ignored while this runs, and the core gives up after 500 ms of host time if
    the firmware never answers.  A CPU stopped in the debugger skips the
    sequence.  Without this hook - as in upstream MAME - the NVRAM file preserves
    the RAM bytes but contains no resumable CPU context, so the next reset takes
    the cold-start path and clears that game state.  When MAME's -autosave option
    is in effect the core skips the sequence, because the state file already
    preserves the complete emulated machine and would otherwise capture the CPU
    sitting in its post-backup halt loop.
    ...
    4000  R  IN1 port, with the main power-off signal synthesised onto
                             bit 7 via shutdown_r() (PORT_CUSTOM_MEMBER)
                             from MAME's own exit request; see above
    ...
    4008  W  status_w()      - status/error-code latch. Wild Cats writes 0x00
                               ...
                               during power failure and, while the power-off
                               sequence is pending, lets the emulation finish
                               powering off. Other possible error codes have
                               not been observed
    ...
    coin counters, and the difference between the two feeds the "inout"
    debug readout on the Debug Overlay view, which the real machine does not
    have.
...
public:
	...
	ioport_value shutdown_r() { return m_shutdown; }
...
private:
	...
	// main power-off signal (IN1 bit 7), raised by power_off_requested()
	...
	// debug readout the real machine does not have: medals taken in minus
	// medals paid out, shown on the Debug Overlay view
	output_finder<> m_inout_sign;
	output_finder<> m_inout;
...
void wildcats_state::payline_lamps_w(uint8_t data)
{
	...
	// debug readout: medals taken in minus medals paid out (see header comment)
	int const medals_in = machine().bookkeeping().coin_counter_get_count(0);
	int const medals_out = machine().bookkeeping().coin_counter_get_count(1);
	m_inout_sign = (medals_in > medals_out) ? 1 : 0;
	m_inout = std::abs(medals_in - medals_out);
}
...
void wildcats_state::status_w(uint8_t data)
{
	...
	// 0x20 while our power-off sequence is pending: the firmware has saved
	// its context, so the emulated machine is now safe to power down
	if ((data == 0x20) && m_shutdown)
		machine().power_off_complete();
}
...
// Called by the core on MAME's own exit request (see machine_start()).
// Just raises the real hardware's main power-off signal, IN1 bit 7; the
// firmware then finishes its backup and writes 0x20 to status_w()
void wildcats_state::power_off_requested()
{
	m_shutdown = true;
}
...
void wildcats_state::machine_start()
{
	...
	// On exit, run the same backup sequence as the real hardware's main
	// power-off.  The firmware's handling is only a few dozen instructions
	// on a 4 MHz Z80, so 500 ms of host time is generous headroom.
	machine().set_power_off_sequence(
			machine_notify_delegate(&wildcats_state::power_off_requested, this),
			attotime::from_msec(500));
	...
	// m_shutdown is the input the Z80 reads through shutdown_r(), so a
	// manual save taken during the power-off wait must restore it as seen.
	// The -autosave path never captures it asserted: the core skips the
	// sequence whenever autosave is in effect.
	save_item(NAME(m_shutdown));
}

void wildcats_state::machine_reset()
{
	m_shutdown = false; // never boot with the main power-off signal asserted
	reset_irq_timer();
...
void wildcats_state::wildcats(machine_config &config)
{
	...
	config.set_default_layout(layout_wildcats_artwork);
}

static INPUT_PORTS_START( wildcats )
	PORT_START("IN1")
	// No manual key for the main power-off signal (bit 7): shutdown_r()
	// reflects MAME's own exit request automatically.  Once raised, the
	// firmware's recheck about 47us later finds it still set and HALTs, and
	// a key release could never bring it back within that window - a manual
	// press would just look like a freeze.
	PORT_BIT( 0x80, IP_ACTIVE_HIGH, IPT_CUSTOM ) PORT_CUSTOM_MEMBER(FUNC(wildcats_state::shutdown_r))  // main power-off signal
